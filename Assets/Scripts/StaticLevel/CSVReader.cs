...
public class CSVReader : MonoBehaviour
{
    ...
    //{
    //    string key = fileName;
    //    if (csvDict.TryGetValue(key, out var value)) return value;
    ...
    public static string[] LoadCSV(string fileName)
    {
        ...
        {
            Debug.LogError("CSV key 是 null 或空字符串");
            return null; // 或 throw / 返回默认值
        }

        if (csvDict.TryGetValue(key, out var cached))
            return cached;
    ...
    using (var request = UnityEngine.Networking.UnityWebRequest.Get(path))
    {
        ...
        {
            Debug.LogError("CSV load failed: " + path);
            return null;
        }
    ...
    float TargetCellIndex(string csvPath, string row, int col)
    {
        ...
        if (colIndex > lines.Length)                    // 行インデックスが CSV 行数の範囲外なら null
        {
        ...
        if (rowIndex > values.Length)                   // 列インデックスが範囲外なら null を返す
        {
