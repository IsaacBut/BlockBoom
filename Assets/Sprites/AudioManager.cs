...
public class AudioManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            instance = this;
            DontDestroyOnLoad(this);    // シーン遷移でも残す
        }
        else
        {
            Destroy(this);              // 2 個目を防止
        }
    }
    ...
    public void ChangeCrip(string name, string cripName)
    {
        ...
        path = Path.Combine(Application.streamingAssetsPath, "StageMusic", cripName);
                ...
                // 実機では StreamingAssets の mp3 は直接読み込めないため、
                // mp3 の代わりに Resources から読み込む（StreamingAssets は Android でファイルアクセスが特殊）
                //
                // Resources.Load() の正しい形：
                //   Resources.Load<AudioClip>("StageMusic/xxx_without_ext");
                //
                // ※ ただし Resources 内の mp3 は Unity が内部で変換し、拡張子を付けずにアクセスする。

                path = Path.Combine("StageMusic", Path.GetFileNameWithoutExtension(cripName));
                AudioClip clip = Resources.Load<AudioClip>(path);

                if (clip == null)
                {
                    Debug.LogError($"[ChangeClip] Resources から BGM を読み込めません: {path}");
                    return;
                }
#endif
    ...
    {
        // =====================================================================
        //  Editor / 開発ビルド：StreamingAssets から mp3 を読み込む
        // =====================================================================
...
#else
        // =====================================================================
        //  Android / iOS（実機）：Resources から読み込むバージョン
        // =====================================================================

        // StreamingAssets 用の path からファイル名だけを取り出し、
        // "StageMusic/ファイル名" のような Resources パスを作成
        ...
        // （実機では mp3 は変換されるので拡張子不要）
        AudioClip clip = Resources.Load<AudioClip>(resourcePath);

        // 見つからない場合はエラー
        if (clip == null)
        {
            Debug.LogError($"[ChangeClip] Resources から BGM を読み込めません: {resourcePath}");
            yield break;
        }

        // 不要な Unload から守る（StreamingAssets 版と統一）
        clip.hideFlags = HideFlags.DontUnloadUnusedAsset;
        // Sound にクリップを登録
        s.clip = clip;
